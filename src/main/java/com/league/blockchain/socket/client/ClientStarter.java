...
import com.league.blockchain.core.bean.Member;
import com.league.blockchain.core.bean.MemberData;
import com.league.blockchain.core.bean.Permission;
import com.league.blockchain.core.bean.PermissionData;
import com.league.blockchain.core.event.NodesConnectedEvent;
import com.league.blockchain.core.manager.PermissionManager;
...
public class ClientStarter {
    ...
    public void fetchOtherServer(){
        ...
        try{
            ...
            MemberData memberData = restTemplate.getForEntity(managerUrl + "member?name=" + name + "&appId=" + AppId
                .value + "&ip=" + localIp, MemberData.class).getBody();
            // 合法的客户端
            if(memberData.getCode()==0){
                List<Member> memberList = memberData.getMembers();
                logger.info("共有" + ((List) memberList).size() + "个成员需要连接: " + memberList.toString());
                nodes.clear();
            ...
            logger.info("请先启动league_blockchain_manager服务，并配置appId等属性，只有合法联盟链成员才能启动该服务");
            System.exit(0);
        }
    }
    ...
    public void onNodeReady(){
        logger.info("开始群发消息获取next Block");
